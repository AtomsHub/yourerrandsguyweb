...
const Order = () => {
  ...
  useEffect(() => {
    const fetchStoredOrder = async () => {
      const storedOrder = await AsyncStorage.getItem("orders");
  ...
  const getOrder = async (showLoader = true) => {
    try {
      ...
      if (response.status === 200) {
        ...
        await AsyncStorage.setItem("orders", JSON.stringify(sortedData));
  ...
  const renderOrderItem = ({ item }) => (
    <View className="px-4">
      <OrderCard order={item} showHeader={false} />
    </View>
  );
                    ...
                    style={{
                      width:
                        stats.totalDeliveries > 0
                          ? `${Math.min((stats.totalDeliveries / 50) * 100, 100)}%`
                          : "5%",
                    }}
                  />
                </View>
          ...
          ListEmptyComponent={
            <View className="items-center justify-center py-20">
              ...
              <Text className="font-Raleway-Regular text-gray-500 text-center w-8/12 leading-5">
                Your order history will appear here once you complete your first
                delivery. Start earning today!
              </Text>
            </View>
          }
